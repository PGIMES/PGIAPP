...
public partial class prod_wip_detail : System.Web.UI.Page
{
    ...
    private void GetData()
    {
        ...
        string sql = string.Format(@"select t.*,hr.tel as cellphone from Mes_App_WorkOrder_Wip t left join  [172.16.5.26].[production].[dbo].[HRM_Emp] hr  on  t.emp_code=hr.employeeid where lot_no= '{0}' and need_no='{1}' ", dh,need_no);
        DataTable dt  = SQLHelper.Query(sql).Tables[0];
        if (dt.Rows.Count<=0)
        {
            sql = string.Format(@"select  distinct sku,sku_descr,qty,sum(off_qty) as off_qty,on_date,hr.tel as cellphone ,feed.b_on_m_emp_name as emp_name 
                                  from Mes_App_WorkOrder_History t
                                     join Mes_App_FeedSku feed on t.need_no=feed.need_no and t.lot_no=feed.lot_no
                                     left join [172.16.5.26].[production].[dbo].[HRM_Emp] hr  on  feed.b_on_m_emp_code=hr.employeeid  where t.lot_no= '{0}' and t.need_no='{1}' 
       ...
       // DataTable dtMainDistinct = dataView.ToTable(true, "pgino", "pn", "workorder","workorder_part","par_qty");//的第一个参数为是否DISTINCT
        ...
        sql = string.Format(@"select t.* ,hr.tel as cellphone from
                            ...
                            join [172.16.5.26].[production].[dbo].[HRM_Emp] hr  on  t.emp_code=hr.employeeid ", dh,need_no);
        dt = SQLHelper.Query(sql).Tables[0];
