...
public partial class MoJu_SB_WX_Chart_APP : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static string init(string type)
    {
        string result = "";
        string sql = @"exec api.dbo.[SheBei_ForMes_Detail_Ver7_chart_APP] '"+ _workshop + "','" + type + "'";

        if (type == "day")
        {
            DataTable dt_day = Query(sql).Tables[0];
            string json_day = JsonConvert.SerializeObject(dt_day);
            result = "[{\"json_day\":" + json_day + "}]";

        }
        else
        {
            DataTable dt_week = Query(sql).Tables[0];
            string json_week = JsonConvert.SerializeObject(dt_week);

            DataTable dt_month = Query(sql).Tables[1];
            string json_month = JsonConvert.SerializeObject(dt_month);

            result = "[{\"json_week\":" + json_week + ",\"json_month\":" + json_month + "}]";
        }
        return result;
